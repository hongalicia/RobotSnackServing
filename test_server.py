...
# 建立 8-byte 的訂單封包
def create_order_packet(peanuts, waffles, id_num):
    packet = bytearray([136, 102, 79, peanuts, waffles, id_num, 0])
...
def handle_robot_messages(client_socket):
    while True:
        try:
            ...
            print("輸入訂單 (栗子 鬆餅): ", end="", flush=True) # 保持輸入提示還在
        except Exception:
            break
...
def main():
    ...
    print("\n=== 模擬點餐發送系統 ===")
    print("請輸入「id 栗子數量 鬆餅數量」，用空白隔開 (例如: 5 1 2)")
    print("輸入 'q' 離開測試程式\n")

    while True:
        user_input = input("輸入訂單 (id 栗子 鬆餅): ")
        ...
        try:
            ...
            packet = create_order_packet(peanuts, waffles, id_num)
            client_socket.sendall(packet)
            print(f"已發送訂單: id={id_num}, 栗子={peanuts}, 鬆餅={waffles}")
